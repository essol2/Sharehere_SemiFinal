...
/**
 * Servlet implementation class User_ReservationList
 */
@WebServlet("/Mypage/User/ReservationList")
public class User_ReservationList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public User_ReservationList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1_1. 게시글 총 갯수 구하기
		int listCount = mp.getUserReservationListCount(userNo);
		...
		// 유저번호로 예약 조회
		ArrayList<ShUserReservation> shUserRes =  mp.selectUserRes(userNo, pi);
		
		
		request.setAttribute("pi", pi);
		request.setAttribute("shUserRes", shUserRes);
		
		// 세션에 넣기
		request.getSession().setAttribute("shUserRes", shUserRes);
