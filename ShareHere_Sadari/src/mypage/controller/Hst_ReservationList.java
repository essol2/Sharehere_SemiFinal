...
/**
 * Servlet implementation class Hst_ReservationList
 */
@WebServlet("/hst/reservationList")
public class Hst_ReservationList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Hst_ReservationList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 1_1. 게시글 총 갯수 구하기
		int listCount = new MyPageService().getHstResListCount(hstNo);
		...
		PageInfo ph = new PageInfo(currentPage, listCount, pageLimit, objectLimit, 
					maxPage, startPage, endPage);
		System.out.println("ph : " + ph);
		
		ArrayList<HstReservation> hstRes =  new MyPageService().selectReservationList(hstNo, ph);
		
		System.out.println("hstRes : "+ hstRes);
		
		request.setAttribute("hstRes", hstRes);
		request.setAttribute("ph", ph);
		
		System.out.println("hstResPh : " + ph);
		
		RequestDispatcher view = request.getRequestDispatcher("/views/Mypage/mp_shReservation.jsp");
