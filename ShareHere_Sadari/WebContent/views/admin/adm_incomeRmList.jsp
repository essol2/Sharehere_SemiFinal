...
<%
	...
	PageInfoForRm pi = (PageInfoForRm)request.getAttribute("pi");
...
<body>
			...
			<%} else { 
				for(Income ic:list) {%>
		            <tr id="fromSecond">
		                <td><%=ic.getPay_no() %></td>
		                <td><%=ic.getUs_no() %></td>
		                <td><%=ic.getPay_item() %></td>
		                <td><%=ic.getPay_date() %></td>
		            </tr>
		    	<%} %>
            <%} %>
    	</table>
	...
	<!-- 페이징 바 -->
		<%-- <div class="pagingArea">
			<!-- 맨 처음으로 (<<) -->
			<% if(s == null){ %>
			<button id="pagingBtn" onclick="location.href='<%= request.getContextPath() %>/income/rm/list?currentPage=1'"> &lt;&lt; </button>
			<% } else { %>
			...
			<%}else if(s == null) { %>
				<button id="pagingBtn" onclick="location.href='<%= request.getContextPath() %>/income/rm/list?currentPage=<%= pi.getCurrentPage() - 1 %>'"> &lt; </button>
			<%} else { %>
			...
			<% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++){ %>
				...
				<%}else if(s == null){ %>
					<button id="pagingBtn" onclick="location.href='<%= request.getContextPath() %>/income/rm/list?currentPage=<%= p %>'"> <%= p %> </button>
				<%} else {%>
			...
			<!-- 맨 끝으로(>>) -->
			<% if(s == null){ %>
			<button id="pagingBtn" onclick="location.href='<%= request.getContextPath() %>/income/rm/list?currentPage=<%= pi.getMaxPage() %>'"> &gt;&gt; </button>
			<% } else { %>
